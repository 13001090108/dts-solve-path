...
//		
	...
	/*
	 * add by lsc 2019/1/12
	 * 根据当前控制流节点得到当前控制流图上节点名称与节点的map映射
	 */
...
//								System.out.println("单重循环:"+  getForPathSingle(vexNode2));
	...
	/*
	 * 根据给定的for循环入口节点获取整个for闭环的路径,注意是获取当前for闭环，仅限于当前(太棒了)
	 * add by lsc 2019/1/12晚
	 */
	public String getForPathSingle(VexNode vexNode) {
		...
		if(vexNode.toString().contains("for_head")) {
			String startforhead = vexNode.toString();
			while(!forpath.contains("for_head")) {
				VexNode vexNode1;
				if(! forpath.isEmpty()) {
	...
	/*
	 * 测试用 add by lsc 2019/1/14早10:24
	 */
	public List<String> getPath_test(VexNode vexNode, List<String> path) {
		...
		if (path.isEmpty()) {
			...
			for (String str : temp) {
				...
				if (str2.contains("while_head")) {
					path.remove(str);

	                strfor = getWhilePathMultiple(map.get(str2));
...
//					temp.add(str);
	...
	/*
	 * 根据给定的for循环入口节点获取整个for闭环的路径,注意此针对多重for循环闭环
	 * 完美解决多重for闭环，add by lsc 2019/1/12/23:21
	 */
					...
					if(vexNode2.getSnumber() < vexNode.getSnumber()) {
						...
						if(vexNode2.toString().contains("for_head")) {
							...
							if(!forpath.contains(getForPathSingle(vexNode2))) {
								//此处注意，应该替换为多重
								forpath += getForPathMultiple(vexNode2);
...
//								System.out.println("单重循环:"+  getForPathSingle(vexNode2));
	...
	/*
	 * add  by lsc 2019/13/11:46
	 * 目的：为了测试用
	 * 功能：根据给定的任意一个控制流图节点获取当前控制流上所有节点与前驱节点的映射
	 */
	 ...
	 * 参数两个： 初始分析的目标节点(即外部人工输入的目标节点),path：目标节点到入口的所有可达路径
	 * 完整解决了包含多重for循环闭环的可达路径计算问题。
	 */
...
//		 System.err.println(vexNode.getGraph());
		 ...
		 for(VexNode vexNode2 : vexNode.getGraph().getAllnodes()) {
			 if(vexNode2.toString().contains("for_head")) {
				 System.err.println("闭环计算结果："+vexNode2+"    " +getForPathMultiple(vexNode2));
			 }
		 }
...
//					temp.add(str);
	...
	/*
	 * 目标节点 ，目标节点前的路径
	 * 完整方法，可用于处理单重for闭环2019/1/12/21:30
	 */
//	public List<String> getPath(VexNode vexNode, List<String> path) {
//
//		// 记录当前控制流图中节点名字和节点的映射
