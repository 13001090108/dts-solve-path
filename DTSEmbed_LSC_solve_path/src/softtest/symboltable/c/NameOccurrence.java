...
import softtest.depchain.c.findAstOfVariable;
...
public class NameOccurrence implements Serializable
{
	...
	private List<String> list_operator ;
    ...
    /** 定义-取消定义链，保存了本定义出现所有可以到达的定义出现(向下找，即将来的定义)，对于使用出现，该链表为null,链表中的元素都为定义出现 */
    private List<NameOccurrence> def_undef = null;
    
    /** 取消定义-定义链，保存了可以到达本定义出现的所有定义出现(向上找，即以前的定义)，对于使用出现，该链表为null,链表中的元素都为定义出现 */
   ...
   // public List<SCVP> scvpList = null;
    ...
    public OccurrenceType checkOccurrenceType(){
    	...
    	//add by lsc 2018/11/24,add += ，-=等
//    	if(isSelfAssignmentChange()) {
//    		type = OccurrenceType.DEF_AFTER_USE;
//    	}
    ...
    /**
     * @author lsc	2018/11/24
     * @return 判断当前变量出现（NameOccurence)是否为自增或自减的变种(+= ,-= , *= , %=)                                                                                                             
     */
    public boolean isSelfAssignmentChange() {
    	if(location instanceof ASTPrimaryExpression){
    		String string = location.jjtGetParent().toString();
	    	if(location.jjtGetParent() instanceof ASTPostfixExpression){
		    	ASTPostfixExpression postfixExp=(ASTPostfixExpression)location.jjtGetParent();
		    	if(postfixExp.jjtGetParent() instanceof ASTUnaryExpression) {
		    		ASTUnaryExpression unaryExpression = (ASTUnaryExpression) postfixExp.jjtGetParent();
		    		if(unaryExpression.jjtGetParent() instanceof ASTAssignmentExpression) {
		    			ASTAssignmentExpression assignmentExpression = (ASTAssignmentExpression) unaryExpression.jjtGetParent();
		    			if(assignmentExpression.containsChildOfType(ASTAssignmentOperator.class)) {
		    				ASTAssignmentOperator assignmentOperator = (ASTAssignmentOperator) assignmentExpression.findDirectChildOfType(ASTAssignmentOperator.class);	
		    				if(assignmentOperator.getOperatorType().contains("+=") )
		    		    	{
		    		    		return true;
		    		    	}
		    			}
